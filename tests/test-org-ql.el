...
(describe "org-ql"
  ...
  (describe "Plain query parsing"
    ...
    (it "Timestamp-based predicates"
      (expect (org-ql--plain-query "scheduled:2017-07-07")
              :to-equal '(and (scheduled :on "2017-07-07")))
      (expect (org-ql--plain-query "deadline:2017-07-07--2017-07-09")
              :to-equal '(and (deadline :from "2017-07-07" :to "2017-07-09")))
      (expect (org-ql--plain-query "planning:2017-07-07--")
              :to-equal '(and (planning :from "2017-07-07")))
      (expect (org-ql--plain-query "closed:2017-07-07-")
              :to-equal '(and (closed :from "2017-07-07")))
      (expect (org-ql--plain-query "ts-active:--2017-07-07")
              :to-equal '(and (ts-active :to "2017-07-07")))
      (expect (org-ql--plain-query "ts-inactive:-2017-07-07")
              :to-equal '(and (ts-inactive :to "2017-07-07")))
      (expect (org-ql--plain-query "ts-a:-2017-07-07")
              :to-equal '(and (ts-a :to "2017-07-07")))
      (expect (org-ql--plain-query "ts-i:2017-07-07")
